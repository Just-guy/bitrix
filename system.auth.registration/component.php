...
//=== Если это не массив и он не пустой, то:
...
//=== Возможна ли регистрация по номеру телефона?
$arResult["PHONE_REGISTRATION"] = (COption::GetOptionString("main", "new_user_phone_auth", "N") == "Y");

//=== Является ли номер телефона обязательным?
...
//=== Интервал повторной отправки телефонного кода
$arResult["PHONE_CODE_RESEND_INTERVAL"] = CUser::PHONE_CODE_RESEND_INTERVAL;

//=== Определяем в какую группу попадет пользователь при регистрации
$def_group = COption::GetOptionString("main", "new_user_registration_def_group", "");

//=== Если имеется группа, то:
if ($def_group != "") {
	//=== Получаем политики безопасности этой группы
	$arResult["GROUP_POLICY"] = CUser::GetGroupPolicy(explode(",", $def_group));
	//===
} else {
	//=== Получаем политики обычного пользователя
...
//=== Если такой страницы не существует, то:
if (defined("AUTH_404")) {
	//=== Направляем на страницу компонента
	$arResult["AUTH_URL"] = POST_FORM_ACTION_URI;
	//===
} else {
	//=== Иначе на страницу регистрации с параметрами $arParamsToDelete
