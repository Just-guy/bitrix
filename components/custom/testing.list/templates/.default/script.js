(function (window) {
	...
	window.JCQuiz = function (params) {
		...
		this.questionElement = document.querySelector('.quiz__question-title');
		this.questionImage = document.querySelector('.quiz__question-image');
		this.submit = document.querySelector('.quiz__button');
		this.ajaxLoader = document.querySelector('.quiz__loader');
		this.quizContainer = document.querySelector('.quiz__container');
		this.quizName = document.querySelector('[name="form_text_1"]');
		this.quizTelephone = document.querySelector('[name="form_text_2"]');
		this.quizForm = document.querySelector('.quiz__form');
		this.quizList = document.querySelector('.quiz__list');
		this.currentQuiz = 0;
		this.score = 0;
		this.answerArr = [];
		...
		inputValidate: function () {
			...
			input.forEach((element) => {
				...
				let value = element.value,
					line = element.closest('.quiz-input');
				for (var i = 0; i < field.length; i++) {
					if (!value) {
						line.classList.add('quiz__line-required');
						setTimeout(function () {
							line.classList.remove('quiz__line-required')
						}.bind(element), 2000);
					}
				}
			});
